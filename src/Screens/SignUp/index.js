import { Box, Button, HStack, NativeBaseProvider, Text, View } from 'native-base';
...
const SignUp = ({ navigation }) => {
  ...
  const signUpHandler = () => {
    ...
    else if(!(userName.value == emptyData || !reg.test(email.value) || phoneNumber.value.length < 10 || password.value.length < 6 || confirmPassword.value != password.value)) {
  ...
  return (
    <NativeBaseProvider>
      <View>
        <Box m={10}>
          <Text fontSize="sm" fontWeight="bold">Sign Up for the best experiance</Text>
          <Text fontSize="xs" color={Color.gray} mt={3} mb={3}>Expereance the all new Shopkart</Text>
          ...
          <HStack mt={2}>
            ...
            <Text fontSize="sm" pl="2" color={Color.blue}
              onPress={() => navigation.navigate('Login')}
            >Existing User? Login</Text>
          </HStack>
        </Box>
      </View>
    </NativeBaseProvider>
